...
class ZKNotaryFlow {
    ...
    open class Client(
        ...
        @Suspendable
        private fun sendAndReceiveNonValidatingWithZKProof(notaryParty: Party, session: FlowSession, signature: NotarisationRequestSignature): UntrustworthyData<NotarisationResponse> {
            val ctx = stx.coreTransaction
            val tx = when (ctx) {
                is ContractUpgradeWireTransaction -> ctx.buildFilteredTransaction()
                is WireTransaction -> buildZKFilteredTransaction(ctx, notaryParty)
                else -> ctx
            }
        ...
        private fun buildZKFilteredTransaction(wtx: WireTransaction, notaryParty: Party): ZKFilteredTransaction {
            ...
            val proof = byteArrayOf()

            return ZKFilteredTransaction(proof, ftx)
        }

        // **************************************************************** //
        // You can probably ignore most of the below, since it is almost
        // a direct copy/paste of the Corda core NotaryFlow.Client
        // **************************************************************** //
        companion object {
